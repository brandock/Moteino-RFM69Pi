/*Brandon Baldock 11/26/2022
This sketch can be used with Moteino USB as a replacement (or addition to) the RFM69Pi hat.

It is exactly the JeeLib Classic RFM12Pi/RFM69Pi firmware. (With the addition of this note.) At this writing it can also be found here: https://github.com/openenergymonitor/RFM2Pi/tree/master/firmware/RFM69CW_RF_Demo_ATmega328

This firmware used the JeeLib Classic radio library, compatible with the original firmware for emonTx V3 and emonTh.

You might use this if you convert your emonPi/emonBase to the LowPowerLabs radio library (compatible with emonTx V4) but need to hear from some devices via the JeeLib Classic format. 

Or, for example, if you replace your RFM69Pi hat with a RFM69SPI hat, and still want a way to communicate with an emonTh running the original firmware.

Some notes about use with Moteino USB.

-It works well to use a USB A Male to Micro Male adapter to connect the Moteino to the emonBase or emonPi.

-After upload, this sketch listens to serial input for updates to the node. For example, send 14i to set the node to 14. 
